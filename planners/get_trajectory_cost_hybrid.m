function [cost, gradient] = get_trajectory_cost_hybrid(x0, trajectory, map, cost_map, cost_map_x, cost_map_y, Ms, D_Fs, A_invs, R_unordereds, localmap, hilbertmap, param)
...
plot(p(1, 1), p(1, 2), 'ro'); hold on;
plot(p(:, 1), p(:, 2), 'r-'); hold on;
plot(p(end, 1), p(end, 2), 'rx'); hold on;
end