...
test_regularizors = [0.0, 1, 5, 10, 20];
...
for j = 2:N
    ...
    hilbertmap.y = y;
    hilbertmap.wt = [];
    params.hilbertmap.regularizor = test_regularizors(j-1);
    hilbertmap_grid = learn_hilbert_map(params, occupancymap, hilbertmap);
    ...
    title(sprintf(' %1.0f', test_regularizors(j-1)));
    xlabel('X [meters]'); ylabel('Y [meters]');
    xticks(1:binmap.XWorldLimits(2)); yticks(1:binmap.YWorldLimits(2));
...
figure(8);
plot(test_regularizors, fp(1, :), '-o'); hold on;
plot(test_regularizors, fp(2, :), '-o'); hold on;
plot(test_regularizors, fp(3, :), '-o'); hold on;
plot(test_regularizors, fp(4, :), '-o'); hold on;
